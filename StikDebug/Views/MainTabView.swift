...
struct LocationSimulationView: View {
    ...
    @State private var customSpeedKmh: Double = 20.0
    ...
    private func rebuildRouteWithCustomSpeed() {
    guard let routePlan else { return }

    let speedMetersPerSecond = customSpeedKmh / 3.6

    routePlaybackSamples = buildPlaybackSamples(
        from: routePlan.displayCoordinates,
        speedWays: [],
        fallbackSpeedMetersPerSecond: speedMetersPerSecond
    )
}
    private func simulateRoute() {
    rebuildRouteWithCustomSpeed()
